...
for module_name in ("detector_runner", "dyfi_poller", "notify", "scorer"):
    stub = types.ModuleType(module_name)
    if module_name in ("detector_runner", "dyfi_poller"):
        stub.main = lambda *args, **kwargs: None
        stub.run = lambda *args, **kwargs: None
    elif module_name == "notify":
        stub.send_event_alert = lambda *args, **kwargs: None
    elif module_name == "scorer":
        stub.main = lambda *args, **kwargs: None
        stub.update_summary = lambda data: data.setdefault("summary", {})
...
def usgs_feature(mag):
    return {
        ...
        "geometry": {"coordinates": [142.25, 38.91, 25.0]},
    }


class CriticalRegressionTests(unittest.TestCase):
    def test_seen_near_miss_can_still_queue_after_usgs_magnitude_upgrade(self):
        queue = {"events": [], "seen_ids": []}

        with patched_attr(usgs_listener, "fetch_feed", lambda: [usgs_feature(6.4)]), patched_attr(
            usgs_listener, "fetch_focal_mechanism", lambda _eid: None
        ), patched_attr(usgs_listener, "_activate_fast_poll", lambda *args, **kwargs: None):
        ...
        with patched_attr(usgs_listener, "fetch_feed", lambda: [usgs_feature(6.7)]), patched_attr(
            usgs_listener, "fetch_focal_mechanism", lambda _eid: None
        ), patched_attr(usgs_listener, "_activate_fast_poll", lambda *args, **kwargs: None):
            new, near_misses = usgs_listener.check_feed(queue)
    ...
    def test_ci_once_exits_even_when_fast_poll_active(self):
        ...
        def forbidden_sleep(_seconds):
            raise AssertionError("CI --once should not sleep or re-run for fast poll")

        with temp_cwd():
            Path("fast_poll.json").write_text(json.dumps(state), encoding="utf-8")
            with patched_env(CI="true"), patched_attr(
                pipeline, "run_pipeline", fake_run_pipeline
            ), patched_attr(pipeline.time, "sleep", forbidden_sleep):
    ...
    def test_discord_webhook_reports_delivery_success(self):
        with patched_env(DISCORD_WEBHOOK_URL=None):
