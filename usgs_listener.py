...
def check_feed(queue):
    ...
    queued_ids = {e.get("usgs_id") for e in queue.get("events", []) if e.get("usgs_id")}
    ...
    for feat in features:
        ...
        first_seen = eid not in queue["seen_ids"]
        if first_seen:
        ...
        if candidate:
            ...
            if reason is not None and (first_seen or mag >= MW_THRESHOLD):
                near_misses.append({
