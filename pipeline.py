    ...
    for _evt in _queue.get("events", []):
        if _evt.get("status") == "predicted" and not _evt.get("discord_alerted"):
            ...
            if delivered:
                ...
                log.warning("Discord detection alert not acknowledged; will retry next cycle")
    for _evt in _queue.get("events", []):
        if _evt.get("retroactive_pending") and (
            _evt.get("status") == "rinex_failed" or _evt.get("retroactive_aborted")
        ):
            delivered = False
            detail = _evt.get(
            ...
            if delivered:
                ...
                log.warning("Discord retro abort alert not acknowledged; will retry next cycle")
