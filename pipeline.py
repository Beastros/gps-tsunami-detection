...
def run_pipeline():
    ...
    for _evt in _queue.get("events", []):
        if (
            _evt.get("retroactive_abort_pending")
            or (
                _evt.get("retroactive_pending")
                and (
                    _evt.get("status") == "rinex_failed"
                    or _evt.get("retroactive_abort_reason")
                )
            )
        ):
            _delivered = False
            try:
                _delivered = notify_discord.send_retroactive_aborted(
                    _evt,
                    _evt.get("retroactive_abort_reason")
                    or "Download returned 0 files; will retry on a later cycle if CDDIS fills in.",
                )
            except Exception as d_err:
                log.warning("Discord retro abort alert failed: %s", d_err)
            if _delivered:
                _evt.pop("retroactive_pending", None)
                _evt.pop("retroactive_abort_pending", None)
        ...
        if once:
            ...
            if _fp.exists():
                try:
                    ...
                    pass
            if _fast and not (os.environ.get("CI") or os.environ.get("GITHUB_ACTIONS")):
            ...
            if _fast:
                log.info("FAST POLL MODE active, but --once is running under CI; exiting after one cycle.")
